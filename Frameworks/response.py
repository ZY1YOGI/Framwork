...
class RenderResponse(HttpResponse):
    ...
    def __init__(self, request: Request, filename: str, context: dict = {}):
        try:
            with open(os.path.join(BASE_DIR, "templates/")+filename, 'r') as f:
                text = f.read()
        except FileNotFoundError:
            print(f'Error openning file {os.path.join(BASE_DIR, "templates/")+filename}')
            raise Exception(f'Could not find the file {os.path.join(BASE_DIR, "templates/")+filename}')
